...
function fetchChannel() {
  return new Promise((resolve, reject) => {
    https.get(`https://t.me/s/${CHANNEL_NAME}`, {
...
function parseMessages(html) {
  const messages = [];
  const msgRegex = /<div class="tgme_widget_message_text js-message_text" dir="auto">([\s\S]*?)<\/div>[\s\S]*?<time class="time" datetime="([\d-T:+]+)">/g;
...
Итоговое резюме:`;
  ...
  return new Promise((resolve, reject) => {
    const args = [
      ...
      '--offline', '--no-skills', '--no-themes', '--no-context-files',
      '-p', prompt
    ];
